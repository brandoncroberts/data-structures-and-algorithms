...
function createProblemDirectory(problemName: string) {
    ...
    files.forEach(file => {
        ...
        if (!existsSync(filePath)) {
            let content = '';
            if (file === 'main.ts') {
                content += `export const ${problemName.replace(/-/g, '_')} = () => {};\n`;
            } else if (file === 'main_test.ts') {
                content += `import { assertEquals } from "jsr:@std/assert";\n`;
                content += `import { ${problemName.replace(/-/g, '_')} } from "./main.ts";\n\n`;
                content += `Deno.test("${problemName.replace(/-/g, '_')} returns correct result", () => {\n`;
                content += `    assertEquals(${problemName.replace(/-/g, '_')}(), 'expected result');\n`;
                content += `});\n`;
    ...
    console.log(`Directory structure for problem "${problemName}" created successfully.`);
}
